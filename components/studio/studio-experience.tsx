...
export function StudioExperience() {
  ...
  return <main id="main-content" tabIndex={-1} className="bg-[#09090b] text-[#f1efe9]">
    <MainHeader />
    ...
    <nav className="sticky top-[78px] z-40 overflow-hidden border-y border-white/15 bg-[#09090b]/95 px-[clamp(18px,5vw,110px)] backdrop-blur-md min-[901px]:top-24" aria-label="Navigation Studio"><div className="flex w-full items-center justify-between gap-3 py-1 text-[12px] font-black tracking-[.1em] text-white/80 min-[901px]:justify-center min-[901px]:gap-10 min-[901px]:py-2 min-[901px]:tracking-[.13em]">{[{ href: "#poles", label: "EXPERTISES" }, { href: "#configurateur", label: "MON PROJET" }, { href: "#accompagnement", label: "SUIVI" }].map((item) => <a className="whitespace-nowrap px-1 py-2 transition hover:text-[#ff66c4] focus-visible:text-[#ff66c4]" href={item.href} key={item.href}>{item.label}</a>)}</div></nav>

    <section id="poles" className={`${section} bg-[#f1efe9] text-[#09090b]`}><div className="grid gap-7 min-[901px]:grid-cols-[1.15fr_.85fr] min-[901px]:items-end"><SectionTitle number="01 / LES TROIS PÔLES">UNE IDÉE.<br /><span className="sr-only"> </span><em className="font-[Georgia] font-normal text-[#ff66c4]">PLUSIEURS FORMES.</em></SectionTitle><p className="max-w-lg text-[15px] leading-[1.7] text-[#514c54]">Design, musique et image réunis pour construire un projet cohérent, de l’intention à sa sortie.</p></div><div className="mt-10 grid gap-3 min-[901px]:grid-cols-3">{poles.map((pole, poleIndex) => <article className="group flex min-h-[350px] flex-col border border-[#c8c2ba] p-5 transition duration-300 hover:-translate-y-1 hover:shadow-[0_24px_55px_rgba(41,20,31,.12)]" key={pole.title}><div className="flex justify-between text-[11px] font-black tracking-[.13em]"><span className="font-[Georgia] text-base italic text-[#ff66c4]">0{poleIndex + 1}</span><span>{pole.price}</span></div><PoleVisual pole={pole.title} /><h3 className="mb-2 mt-5 text-[clamp(34px,4vw,50px)] tracking-[-.06em] transition group-hover:text-[#ff66c4]">{pole.title}</h3><p className="mb-0 text-[13px] leading-[1.6] text-[#514c54]">{pole.items.slice(0, 4).join(" · ")}</p><a className="mt-auto pt-3 text-[11px] font-black tracking-[.13em] text-[#d91c86]" href="#configurateur">COMPOSER MON PROJET</a></article>)}</div><p className="mt-5 text-[12px] leading-[1.6] text-[#5e5961]">Tarifs de départ indicatifs. L’estimation détaillée se construit dans le configurateur.</p></section>

    <section id="configurateur" className={`${section} scroll-mt-24 bg-[#f1efe9] text-[#09090b]`}><SectionTitle number="02 / CONSTRUIRE MON PROJET">TON PROJET.<br /><span className="sr-only"> </span><em className="font-[Georgia] font-normal text-[#ff66c4]">À TA MANIÈRE.</em></SectionTitle><p className="mt-7 max-w-xl text-[15px] leading-[1.7] text-[#514c54]">Pars d’une base ou compose librement. Le tarif et le récapitulatif se mettent à jour immédiatement.</p><div className="mt-9 grid gap-2 min-[581px]:grid-cols-3" aria-label="Bases de projet rapides">{presets.map((preset) => <button className="min-h-14 border border-[#aaa4ac] bg-transparent px-4 text-left text-[12px] font-black tracking-[.08em] transition hover:border-[#ff66c4] hover:bg-[#ff66c4]/10" type="button" onClick={() => choosePreset(preset)} key={preset.type}><span className="block text-[#d91c86]">BASE {preset.type.toUpperCase()}</span><small className="mt-1 block text-[11px] font-medium normal-case tracking-normal text-[#5e5961]">{preset.text}</small></button>)}</div><div className="mt-12 grid items-start gap-12 min-[901px]:grid-cols-[1.2fr_.8fr]">
